...
const handleUpdateInvoice = async () => {
  try {
    const subtotal = editInvoiceForm.items.reduce((sum, item) => 
      sum + (item.quantity * item.unit_price), 0
    );
    const tax_amount = subtotal * (editInvoiceForm.tax_rate / 100);
    const discount_amount = editInvoiceForm.discount;
    const total_amount = subtotal + tax_amount - discount_amount;

    await api.put(`/financial/invoices/${selectedInvoice.id}`, {
      ...editInvoiceForm,
      subtotal,
      tax_amount,
      discount_amount,
...
<Dialog open={openViewInvoiceDialog} onClose={() => setOpenViewInvoiceDialog(false)} maxWidth="md" fullWidth>
  <DialogTitle>
    ...
    {selectedInvoice?.status !== 'paid' && !editingInvoice && (
      <Button
        size="small"
      ...
      <Box>
        {editingInvoice ? (
          ...
          <Grid container spacing={2}>
            ...
            {/* Add similar fields for items, tax, discount, etc. */}
          </Grid>
        ) : (
          // View Mode (keep existing view code)
          <Grid container spacing={2}>
            {/* ... existing view code ... */}
          </Grid>
        )}
  ...
  <DialogActions>
    {editingInvoice ? (
      <>
        <Button onClick={() => setEditingInvoice(false)}>Cancel</Button>
        <Button onClick={handleUpdateInvoice} variant="contained">Save Changes</Button>
      </>
    ) : (
      <>
